// Name: poker_hand4.c
// Purpose: To extend the poker hand program to include ace low straights.
// Author: Ayush Raj
...
void analyze_hand(void) {
    int consecutive_ranks = 0, copy = 0, same_suit = 0, same_rank = 0;

    for (int i = 0; i < NUM_RANKS; i++) {
        if (num_in_rank[i])
            consecutive_ranks++;
        else if (consecutive_ranks < NUM_CARDS) {
            if (consecutive_ranks > copy) {
                copy = consecutive_ranks;
            }
            consecutive_ranks = 0;
        }
    ...
    if (consecutive_ranks == NUM_CARDS || (copy == 4 && !num_in_rank[9] && num_in_rank[0] && num_in_rank[12]))
        straight = true;
